...
switch val
    ...
    % Weickert and Scharr weights
      ...
      % Non negative scheme
      r = 1/4*((abs(bmp)-bmp)+(abs(b)-b)).*Lmp+...
          1/2*(ccp+c-abs(bcp)-abs(b)).*Lcp+...
          1/4*(abs(bpp)+bpp+abs(b)+b).*Lpp+...
          1/2*(amc+a-abs(bmc)-abs(b)).*Lmc+...
          (1/2*(-amc-2*a-apc)-1/4*(abs(bmp)-bmp+abs(bpp)+bpp)- ...
          1/4*(abs(bmm)+bmm+abs(bpm)-bpm)+ ...
          1/2*(abs(bmc)+abs(bpc)+abs(bcm)+abs(bcp)+ ...
          2*abs(b))-1/2*(ccm+2*c+ccp)).*L+...
          1/2*(apc+a-abs(bpc)-abs(b)).*Lpc+...
          1/4*(abs(bmm)+bmm+abs(b)+b).*Lmm+...
          1/2*(ccm+c-abs(bcm)-abs(b)).*Lcm+...
          1/4*(abs(bpm)-bpm+abs(b)-b).*Lpm;
